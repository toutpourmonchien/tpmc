...
var defaultLayer = {
    ...
    outgoingclasses: 'randomrotateout',
    data_start: 1000,
    data_end: 0,
    data_easing: 'easeOutBack',
    data_endeasing: 'easeOutBack',
    removed: 0,
    video_width: 600,
    video_height: 400,
    width: 200,
...
(function ($) {
    ...
    function addLayerTab(layerIndex) {
        ...
        var layerTabTitle = $('<span>').text('Layer #' + (layerIndex + 1));
        ...
        if (typeof $slides[$currentSlide].layers[layerIndex].text_style == 'undefined') {
            $slides[$currentSlide].layers[layerIndex].text_style = 'medium_text';
        }
        ...
        if ($slides[$currentSlide].layers[layerIndex].type != 'image') {
            //newLayerDesign.width($slides[$currentSlide].layers[layerIndex].width || 100);
            //newLayerDesign.height($slides[$currentSlide].layers[layerIndex].height || 50);
        }
        var content = '';
        ...
        case 'image':
            ...
            break;
        case 'video':
            content = '<img width="' + $slides[$currentSlide].layers[layerIndex].width + '" height="' + $slides[$currentSlide].layers[layerIndex].height + '" src="' + Drupal.settings.basePath + 'sites/all/modules/dexp_layerslider/images/video.jpg"/>';
            break;
        case 'text':
        ...
        $('#layeroptions').show(0);
        try{$('#layerslist').sortable('destroy');}catch(e){}
        /*
				$('#layerslist').sortable({
            handle: '.move',
            update: function (event, ui) {
            ...
            select: function (event, ui) {
                ...
                if (type == 'image') {
                    ...
                    var videothumbnail = $('<img>').attr({
                        src: Drupal.settings.basePath + 'sites/all/modules/dexp_layerslider/images/video.jpg',
    ...
    function saveLayerSlider(ajax) {
        ...
        $('#save').val('Saving...');
        $.ajax({
            url: Drupal.settings.basePath + 'admin/dexp_layerslider/save',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function (data) {
